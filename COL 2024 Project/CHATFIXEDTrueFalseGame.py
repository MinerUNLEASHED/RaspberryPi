...
def continueGame1():
    ...
    ct += 1
    if ct <= 7:
        playQ(questions[ct % 5], answers[ct % 5])
    else:
        ending()

def ending():
    welcome_txt.config(text="Thanks for playing!")
    root.after(3000, startAll)
...
author_txt = tk.Label(root, text="Aalim, Aalam, Alyan, And Azhmeer 2024", font=("Raleway", 30), bg=BgString, fg=FgString)
...
start_btn = tk.Button(root, text='Start Playing', height=4, width=30, command=lambda: startGame(), bg="#c7eae4", activebackground="#ffd972", fg="#000000", activeforeground='#FFFFFF', font=('Raleway',20))
